...
while running_game:
  for event in pygame.event.get():
    ...
    if event.type == pygame.MOUSEWHEEL:
      if event.y > 0:
        MM_Settings.TILE_SIZE += 1
      else:
        if MM_Settings.TILE_SIZE > 1:
          MM_Settings.TILE_SIZE -= 1
  ...
  if filters.show_grid:
    if pygame.key.get_pressed()[pygame.K_UP]:
      MM_Settings.TILE_SIZE += 1
      canvas.update_grid_tiles()
    if pygame.key.get_pressed()[pygame.K_DOWN] and MM_Settings.TILE_SIZE > 1:
      MM_Settings.TILE_SIZE -= 1
    canvas.update_grid_tiles()
  ...
  if pallet.curr_pensil == "enemy":
    ...
    sprite_demo = pygame.transform.scale(
      sprite_demo, (MM_Settings.TILE_SIZE, MM_Settings.TILE_SIZE)
    )
